...
def create():
  ...
  if request.method == 'POST':     #O método será post quando o usuário enviar as informações do formulário para se registrar
    ...
    if user_check: 
      return render_template('usererror.html'), 400                    #Se não for vazio, existe um usuário com esse nome. Então, um arquivo html é aberto dizendo "usuário já cadastrado" e com um link de volta ao início
